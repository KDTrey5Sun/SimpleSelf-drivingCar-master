...
def draw_track(win):
    ...
    # -------- 水平赛道右端封口红黄护栏（竖向条纹，彻底封闭赛道右端） --------
    end_x = WIDTH - LANE_WIDTH
    for j in range(0, TRACK_WIDTH, 40):
        color = RED if (j // 40) % 2 == 0 else YELLOW
        seg_h = min(40, TRACK_WIDTH - j)
        pygame.draw.rect(win, color, (end_x, horizontal_start_y + j, LANE_WIDTH, seg_h))
    ...
    # -------- 以终点线中心为圆心的650像素半径圆圈 --------
    finish_center_x = CENTER_X
    finish_center_y = finish_line_y + square_size  # 终点线中心Y坐标
    circle_radius = 500
    pygame.draw.circle(win, WHITE, (finish_center_x, finish_center_y), circle_radius, 2)

    # -------- 小车起点标记 --------
    # 计算小车起点位置（与 autocar_v4.py 中的 ComputerCar 起点一致）
    START_SHIFT_X = 200
    base_start_x = CENTER_X + 400
    base_start_y = HEIGHT // 2 + TURN_RADIUS - TRACK_WIDTH // 2
    car_start_x = base_start_x + START_SHIFT_X
    car_start_y = base_start_y
    
    # 绘制起点标记：绿色圆圈
    GREEN = (0, 255, 0)
    pygame.draw.circle(win, GREEN, (int(car_start_x), int(car_start_y)), 15, 3)
    # 在起点圆圈中心画一个小点
    pygame.draw.circle(win, GREEN, (int(car_start_x), int(car_start_y)), 3)
