...
SURVIVE_REWARD = 0.2        # 轻量生存
PROGRESS_GAIN = 10           # 轻量进度（避免主导）
CENTER_GAIN = 12            # 居中奖励（越居中越高）
ALIGN_GAIN = 8             # 朝向对齐奖励（越对准终点越高）
...
# 无进展判定与上限
PROGRESS_THRESH = 0.4   # 低于该进度视为“无进展”
NO_PROGRESS_LIMIT = 60  # 连续无进展步数上限
NO_PROGRESS_TERM_PENALTY = 300  # 终止时的额外小惩罚
...
# 车辆图片（绝对路径，按需调整）
CAR_IMAGE_PATH = '/Users/smz/Desktop/SimpleSelf-drivingCar-master/imgs/green-car.png'
CAR_IMAGE = scale_image(pygame.image.load(CAR_IMAGE_PATH), 0.3)
...
class ComputerCar(AbstractCar):
    ...
    def step(self, action):
        ...
        if self.prev_finish_dist is None:
            # 首步或未初始化时，将前距离设为当前距离（进度为0）
        ...
        reward += center_reward + align_reward + progress_reward + SURVIVE_REWARD
