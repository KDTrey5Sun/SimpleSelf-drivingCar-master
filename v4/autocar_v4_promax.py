...
# 速度与中线相关奖励参数
CENTER_GAIN = 150.0         # 靠近中线的奖励权重（幅度型）
SPEED_GAIN = 2.0            # 速度线性奖励系数
SPEED_CENTER_SYNERGY = 3.0  # 速度×居中 协同奖励系数
MIN_SPEED = 2.5             # 最低速度阈值（像素/帧）
LOW_SPEED_LIMIT = 60        # 低速连续步数上限
LOW_SPEED_TERM_PENALTY = 400  # 低速提前终止惩罚
WRONG_WAY_COEF = 30.0       # 远离终点的即时扣分系数（progress<0）
# 低速检测热身期：在前 N 步不触发低速计数，避免任务起步阶段被提前终止
LOW_SPEED_WARMUP_STEPS = 120

# 训练稳定性与引导
# - 更温和的“停滞”惩罚，避免策略被强负反馈困住
# - 略放宽“无进展”的判定阈值并延长上限，允许小步推进
# - 增加朝向对齐奖励，帮助早期朝向正确
STALL_SPEED_DELTA_THRESH = 0.2  # 位移阈值（像素/步）
STALL_PENALTY = 80.0            # 停滞惩罚（由原 -200 降低）
PROGRESS_THRESH = 0.25          # 判定“有进展”的阈值（由 0.5 降低）
NO_PROGRESS_LIMIT = 120         # 无进展累计上限（由 80 提高）
NO_PROGRESS_TERM_PENALTY = 200  # 无进展终止扣分（由 300 降低）
ALIGN_GAIN = 4.0                # 朝向对齐奖励权重（0~4）
...
class ComputerCar(AbstractCar):
    ...
    def step(self, action):
        ...
        # 保持中线高速：生存 + 速度 + 居中 + 速度×居中 协同 + 位移推进 / 停滞惩罚
        distance_delta = self.get_distance_delta()
        border_dist = self.get_distance_to_border()
        center_dist = MID_TRACK - border_dist
        center_dev = min(1.0, abs(center_dist) / MID_TRACK)  # [0,1]
        center_reward = CENTER_GAIN * (1.0 - center_dev)

        reward += 1.0
        reward += SPEED_GAIN * self.vel
        reward += center_reward
        reward += SPEED_CENTER_SYNERGY * self.vel * (1.0 - center_dev)

        if distance_delta > STALL_SPEED_DELTA_THRESH:
            reward += 3.0 * distance_delta
        else:
            reward -= STALL_PENALTY

        # 朝向对齐奖励：与终点方向的夹角越小奖励越高（0~ALIGN_GAIN）
        finish_x, finish_y = CENTER_X, 40
        dx, dy = finish_x - self.x, finish_y - self.y
        angle_diff = (math.degrees(math.atan2(dy, dx)) - self.angle + 180) % 360 - 180
        align_factor = 1.0 - min(1.0, abs(angle_diff) / 90.0)  # |角度|<=90 时线性奖励，超出90不再加分
        reward += ALIGN_GAIN * align_factor
        ...
        if progress < 0:
            reward += WRONG_WAY_COEF * progress
        ...
        # 低速提前终止（热身 + 与“无进展”联动）
        # 思路：
        # 1) 前 LOW_SPEED_WARMUP_STEPS 步不统计低速，防止任务必须的慢起步阶段被误杀。
        # 2) 只有在“持续低速”且“同时无进展计数达到一定阈值”时才触发，避免在过弯等需要慢速但仍在推进时被终止。
        if self.step_count >= LOW_SPEED_WARMUP_STEPS:
            if self.vel < MIN_SPEED:
                self.low_speed_steps += 1
            else:
                self.low_speed_steps = 0
            # 要求无进展有一定积累（例如 >= 10 步），再配合低速超限才终止
            if self.low_speed_steps >= LOW_SPEED_LIMIT and self.no_progress_steps >= 20:
                reward -= LOW_SPEED_TERM_PENALTY
                done = True
                self.termination_reason = 'low_speed'
                return self.get_state(), reward, done
        else:
            # 热身期内不累加低速计数
            self.low_speed_steps = 0
