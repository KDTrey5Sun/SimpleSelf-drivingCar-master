...
def _select_device() -> T.device:
    # 新增：环境变量控制强制使用 CPU，便于多进程并行避免争抢 GPU
...
class Agent:
    ...
    def save_model(self, path: str):
        state = {'Q_eval': self.Q_eval.state_dict(),'Q_next': self.Q_next.state_dict(),
                 'epsilon': self.epsilon,'iter_cntr': self.iter_cntr}
        T.save(state, path)
    ...
    def load_model(self, path: str, strict: bool = True):
        ...
        if 'epsilon' in state: self.epsilon = float(state['epsilon'])
        if 'iter_cntr' in state: self.iter_cntr = int(state['iter_cntr'])

    def choose_action(self, observation):
        if np.random.random() > self.epsilon:
            with torch.inference_mode():
                state_np = np.asarray(observation, dtype=np.float32)
                state = T.as_tensor(state_np, device=self.Q_eval.device).unsqueeze(0)
    ...
    def learn(self):
        ...
        new_states = T.as_tensor(new_states, dtype=T.float32, device=self.Q_eval.device)
        rewards = T.as_tensor(rewards, dtype=T.float32, device=self.Q_eval.device)
        if self.clip_reward: rewards = rewards.clamp(-1.0, 1.0)
        dones = T.as_tensor(dones, dtype=T.bool, device=self.Q_eval.device)
        actions = T.as_tensor(actions, dtype=T.long, device=self.Q_eval.device)
