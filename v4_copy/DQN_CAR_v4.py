...
def run_v4_training(
    ...
    output_curve='./v4_copy/exp_data/curve_data_v4.txt',
    output_summary='./v4_copy/exp_data/summary_v4.txt',
    output_log='./v4_copy/exp_data/train_log_v4.txt',
    print_every=1,
        ...
        while success_count < max_success:
            ...
            if succeeded:
                ...
                log_print(log_f,
                    ...
                    f"steps {step_count} | {tries_note} | {took*1000:.0f} ms"
                )
            else:
            ...
            if now - last_ckpt_time >= ckpt_interval_sec:
                ckpt_path = os.path.join(ckpt_dir, f"ckpt_time_{int(now)}.pt")
                try:
            ...
            if (episode_idx % print_every) == 0:
                log_print(log_f,
                    ...
                    f"steps {step_count:4d} | {tries_note} | {took*1000:.0f} ms"
                )
